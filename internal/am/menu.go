...
// WIP: These styles are expected to be configurable by editing some SCSS when the assets pipeline is in place.
// We also don't want to enforce TailwindCSS at this level, so these are temnporary until a better solution is found.
const (
...
func (m *Menu) AddListItem(resource Identifiable, text ...string) {
	action := fmt.Sprintf("list-%s", Plural(resource.Type()))
	btnText := "Back"
	if len(text) > 0 {
		btnText = text[0]
	}
...
func (m *Menu) AddNewItem(r Identifiable, text ...string) {
	action := fmt.Sprintf("new-%s", r.Type())
	btnText := "New"
	if len(text) > 0 {
		btnText = text[0]
	}
...
func (m *Menu) AddShowItem(i Identifiable, text ...string) {
	action := fmt.Sprintf("show-%s", i.Type())
	btnText := "Show"
	if len(text) > 0 {
		btnText = text[0]
	}
...
func (m *Menu) AddEditItem(i Identifiable, text ...string) {
	action := fmt.Sprintf("edit-%s", i.Type())
	btnText := "Edit"
	if len(text) > 0 {
		btnText = text[0]
	}
...
func (m *Menu) AddDeleteItem(i Identifiable, text ...string) {
	action := fmt.Sprintf("delete-%s", i.Type())
	btnText := "Delete"
	if len(text) > 0 {
		btnText = text[0]
	}
